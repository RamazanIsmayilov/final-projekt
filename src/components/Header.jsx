...
const Header = () => {
  ...
  const closeSearch = () => {
    searchGroup.classList.add('close-search');
  }

  const cartFunc = () => {
    localStorage.getItem("login") === 'true' ? navigate("/cart") : navigate("/login")
    closeMenu()
  }
        ...
        <div className="container">
          <nav className="navbar py-3">
            <div className="navbar-left d-flex gap-4">
              <Link className="title" to="/">{lang ? "Ana səhifə" : "Home"}</Link>
              <Link className="title" to="/about">{lang ? "Haqqında" : "About"}</Link>
              <Link className="title" to="/contact">{lang ? "Əlaqə" : "Contact"}</Link>
              <Link className="title" to="/products">{lang ? "Mağaza" : "Store"}</Link>
              <Link className="title" to="/blog">{lang ? "Bloq" : "Blog"}</Link>
            </div>
            ...
            <div className="navbar-right d-flex align-items-center gap-3">
              ...
              <div className="cart d-flex align-items-center gap-2">
                ...
                <button onClick={openCartMenu} className="btn position-relative p-0">
                ...
                <div className="cart-menu" ref={cartMenu}>
                  <div className="heading d-flex align-items-center justify-content-between px-2 py-1">
                    <div className='d-flex align-items-center gap-2'>
                      ...
                      <h4 className='text-light fw-bold mt-1'>{lang ? "Mənim Səbətim" : "My Cart"}</h4>
                    </div>
                    <span className='fw-bold text-light'>{totalItems} {lang ? "MƏHSUL" : "ITEMS"}</span>
                  </div>
                  <div className="body p-3">
                    {isEmpty ? <div className='text-center my-3'><BsCartX style={{ fontSize: "80px" }} /><p className='fw-bold mt-3 fs-6'>{lang ? "Sizin səbətiniz boşdur" : "Your cart is empty  "}
                    </p></div> : <div>
                      {items.map(item => (
                  ...
                  <div className="footer">
                    <div className="container">
                      ...
                      <div className="input-group mt-4">
                        ...
                        <button onClick={closeCartMenu} className="btn btn-secondary px-4" type='button'>{lang ? "Müraciət edin" : "Apply"}</button>
                      </div>
                      <div className="subtotal d-flex align-items-center justify-content-between mt-3">
                        <h4 className='fw-bold'>{lang ? "Toplam qiymət" : "Subtotal"}:</h4>
                        <h3 className='fw-bold'>{Math.round(cartTotal)}.00USD</h3>
                      </div>
                      <div className="footer mt-4">
                        <Link onClick={closeCartMenu} to="/checkout" className='button gap-2 fw-bold d-flex align-items-center justify-content-center mx-5'>{lang ? "Yoxlamaya davam edin" : "Proceed to checkout"}
                        ...
                        <Link onClick={closeCartMenu} to="/cart" className='text-dark gap-2 fw-bold d-flex align-items-center justify-content-center mx-5 mt-3'>{lang ? "Səbətə Baxın" : "View Cart"}
              ...
              <div ref={menu} className="offcanvas-menu">
                ...
                <div className="body">
                  <div className="nav-item d-flex flex-column gap-4 mx-4 py-4">
                    <Link onClick={closeMenu} className="title" to="/">{lang ? "Ana səhifə" : "Home"}</Link>
                    <Link onClick={closeMenu} className="title" to="/about">{lang ? "Haqqında" : "About"}</Link>
                    <Link onClick={closeMenu} className="title" to="/contact">{lang ? "Əlaqə" : "Contact"}</Link>
                        ...
                        <div className="user accordion-body">
                          {localStorage.getItem('login') === 'true' ? <div className="links d-flex flex-column gap-2">
                            ...
                            <button className='logoutbtn p-0 d-flex align-items-center justify-content-start' onClick={() => {
                              localStorage.setItem("login", 'false');
                              window.location.assign('/login');
                            }}>{lang ? "Çıxış" : "Log out"}</button>
                            <Link to="/login">{lang ? "Daxil ol" : "Log in"}</Link>
                            <Link to="/createaccount">{lang ? "Hesab yarat" : "Create Account"}</Link>
        ...
        <div className="container">
          <div className="row d-flex align-items-center">
            <div className="col-12 col-sm-12 col-md-4 col-lg-4">
              <div className="left">
                <span className='fw-bold'>{lang ? "Yeni Aksesuarlara -30% endirim." : "New Accessories -30 % Off."} <Link to="/products">{lang ? "Daha çox" : "More"}</Link></span>
              </div>
            </div>
            <div className="col-12 col-sm-12 col-md-4 col-lg-4">
            ...
            <div className="col-12 col-sm-12 col-md-4 col-lg-4">
              <div className="right d-flex justify-content-end">
                <Link to="/products">{lang ? "Yeni Kolleksiyalar" : "New Collections"}</Link>
              </div>
            </div>
          </div>
        </div>
